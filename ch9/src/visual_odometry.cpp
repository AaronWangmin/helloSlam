...
void VisualOdometry::featureMatching()
{
    ...
    std::cout << "match cost time: " << timer.elapsed() << std::endl;
}
...
void VisualOdometry::poseEstimationPnP()
{
    ...
    BlockSolver_6_2* solver_ptr = new BlockSolver_6_2( linearSolver);
    ...
    for (int i = 0; i < inliers.rows; i++)
    {
        ...
        // 3D -> 2D projection
        EdgeProjectXYZUVPoseOnly* edge = new EdgeProjectXYZUVPoseOnly();
        edge->setId( i );
    ...
    std::cout << "T_c_w_estimated: " << std::endl << T_c_w_estimated_.matrix() << std::endl;
    ...
    if ( map_->keyframes_.empty())
    {
        ...
        for (size_t i = 0; i < keypoints_curr_.size(); i++)
        {
            ...
            MapPoint::Ptr map_point = MapPoint::createMapPoint(
                    p_world, n, descriptors_curr_.rows(i).clone(), curr_.get()
                );
...
void VisualOdometry::addMapPoints()
{
    ...
    for ( int i = 0; i < keypoints_curr_.size(); i++)
    {
        ...
        n.normalize();
        MapPoint::Ptr map_point = MapPoint::createMapPoint(
            p_world, n, descriptors_curr_row(i).clone(), curr_.get()
        );
